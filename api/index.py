...
def conectar_bd():
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro de infraestrutura BD: {str(e)}")
...
def registrar_novo_usuario_unidade(dados: AtualizarUsuarioSchema):
    ...
    try:
        ...
        db.close()
        raise HTTPException(status_code=400, detail="Esta unidade já existe.")
    db.close()
    return {"status": "sucesso", "mensagem": "Unidade ativada."}
...
def receber_lote_medicamento(lote: LoteMedicamentoSchema):
    ...
    cursor.execute(
        "INSERT INTO lotes (medicamento_id, numero_lote, fabricante, data_fabricacao, validade, quantity, usuario_dono) VALUES (%s,%s,%s,%s,%s,%s,%s)",
